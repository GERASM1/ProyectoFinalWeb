...
app.get("/variables", (req, res, next) => {
    res.statusMessage = "variables enviadas";
        ...
        .then(userFound => {
        	if(userFound){
        		if(bcrypt.compareSync(password, userFound.pass)){
		            loggedUserName = userFound.name;
                    loggedUserID = userFound.id;
                    loggedAdmin = userFound.principal;
    ...
    UserList.emailExists(email).then(function(valid) {
        if (!valid) {
            ...
            UserList.createUser(returnedUser)
                    .then(User => {
                        loggedUserName = returnedUser.name;
                        loggedUserID = returnedUser.id;
                        loggedAdmin = returnedUser.principal;
